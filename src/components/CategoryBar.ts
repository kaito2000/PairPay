...
export function renderCategoryBar(expenses: Expense[]): string {
  ...
  if (total === 0) {
    return `
      <div class="bg-white rounded-[24px] p-4 border border-[#eeebe4] shadow-soft">
        <div class="text-xs font-bold text-[#3a3f3b] mb-1.5">カテゴリ別内訳</div>
        <div class="text-xs text-[#8f9690] py-3 text-center font-medium">未精算の支出はありません</div>
      </div>
    `;
  }
  ...
  return `
    <div class="bg-white rounded-[24px] p-4 border border-[#eeebe4] shadow-soft">
      <div class="flex items-center justify-between mb-3">
        <span class="text-xs font-bold text-[#3a3f3b]">カテゴリ別内訳</span>
        <span class="text-[11px] font-semibold text-[#808781]">未精算 計 ¥${total.toLocaleString()}</span>
      </div>
      ...
      <div class="h-2.5 w-full rounded-full bg-[#f2efe9] overflow-hidden flex mb-3.5">
        ${items
          .map(
            (item) => `
          <div style="width: ${item.percent}%; background-color: ${item.category.color};" class="h-full transition-all duration-300" title="${item.category.label}: ${item.percent}%"></div>
        `
          )
          .join('')}
      </div>
      ...
      <div class="flex flex-wrap gap-2">
        ${items
          ...
          <div class="flex items-center gap-1.5 text-[11px] bg-[#f9f8f6] border border-[#ece8e1] px-2.5 py-1 rounded-lg">
            ...
            <span class="text-[#7c827d] font-bold">${item.percent}%</span>
          </div>
        `
          )
          .join('')}
