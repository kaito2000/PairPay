...
export function renderSettlementCard(summary: SettlementSummary, household: Household): string {
  ...
  return `
    <div class="bg-gradient-to-br from-[#2f3e37] via-[#28362f] to-[#202b26] text-white rounded-[28px] p-5 shadow-card relative overflow-hidden">
      ...
      <div class="flex items-center justify-between mb-4 relative z-10">
        <span class="text-[11px] font-bold tracking-wider text-[#a3b899] uppercase">今月の精算サマリー</span>
        <span class="text-[11px] text-[#e8f0e6] font-semibold bg-white/10 backdrop-blur-md px-2.5 py-0.5 rounded-full border border-white/15">
          折半比率 ${household.ratio_user1}:${household.ratio_user2}
        </span>
      </div>
      ...
      <div class="mb-4 p-3.5 rounded-2xl ${
        ...
        <div class="text-[11px] text-[#b7d5be] font-medium mb-1">
          ${isZero ? '精算状況' : '送金アクション'}
        </div>
        <div class="text-base font-black flex items-center justify-between">
          <span class="flex items-center gap-2">
            ${
              isZero
                ? '<i data-lucide="check-circle" class="w-4 h-4 text-[#a3b899] inline"></i> 差額なし・精算完了'
        ...
        <div class="mt-1 text-[11px] text-[#c0d4c8]">
          未精算の総支出: <span class="text-white font-bold">¥${summary.totalAmount.toLocaleString()}</span>
        </div>
      </div>

      <!-- アクションボタン -->
      <div class="grid grid-cols-2 gap-2 relative z-10">
        <button id="btn-copy-line" class="flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-2xl bg-white/10 hover:bg-white/15 active:scale-[0.98] text-xs font-bold text-white border border-white/15 transition-all shadow-xs cursor-pointer">
          <i data-lucide="share-2" class="w-3.5 h-3.5 text-[#b7d5be]"></i>
          <span>LINE文面コピー</span>
        </button>
        <button id="btn-settle-all" class="flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-2xl bg-gradient-to-r from-[#52796f] to-[#406259] hover:from-[#5e8a7f] hover:to-[#4a6f65] active:scale-[0.98] text-xs font-bold text-white shadow-md shadow-[#202b26]/30 transition-all cursor-pointer">
